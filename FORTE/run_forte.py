...
def _world_offset_from_strategy(cs: ContactStrategy) -> Optional[np.ndarray]:
    world_offset = cs.metadata.get("world_offset")
    if world_offset is None:
        return None
    return np.asarray(world_offset, dtype=np.float64).reshape(3)
...
def _desired_contact_from_strategy(wrapper: ForteWrapper, cs: ContactStrategy) -> np.ndarray:
    return compute_box_face_anchor(
        ...
        world_offset=_world_offset_from_strategy(cs),
    )


def _face_axis_sign_order(wrapper: ForteWrapper, base: ContactStrategy) -> List[Tuple[int, float]]:
    """Ordered face candidates: semantic seed, geometry fallback, then wall-aware signs."""
    box_rot = wrapper.get_box_rotmat()
    wall_pos = wrapper.get_wall_pos()
    box_pos = wrapper.get_box_pos()
    geom_axis, geom_sign = compute_best_approach_face(box_rot, wall_pos, box_pos)
    ...
    for axis in (0, 1, 2):
        sign = compute_approach_face_sign(box_rot, wall_pos, box_pos, face_axis=axis)
        ordered.append((axis, float(sign)))
    seen = set()
...
def _select_contact_hypothesis(
    ...
    base: ContactStrategy,
    *,
    measured_force_normal: float,
    force_band_lower: float,
    force_band_upper: float,
) -> Tuple[ContactHypothesis, List[ContactHypothesis]]:
    """Physics filter + tracking-first rerank (semantic is a light tie-breaker)."""
    band_mid = 0.5 * (float(force_band_lower) + float(force_band_upper))
    ...
    for cs in _candidate_strategies(wrapper, base):
        ...
        semantic_penalty = 0.0 if same_semantic_face else (0.12 * semantic_conf)
        geom_bonus = -0.08 if (same_geom_face and eef_dist < 0.12) else 0.0
        score = 3.0 * eef_dist + 0.8 * force_error + 0.3 * wall_gap + semantic_penalty + geom_bonus
...
def _geometric_fallback_strategy(wrapper: ForteWrapper, base: ContactStrategy) -> ContactStrategy:
    ...
    return ContactStrategy(
        ...
        contact_vertical_offset_scale=base.contact_vertical_offset_scale,
        gripper_command=base.gripper_command,
    ...
    *,
    ...
    try:
        for step in range(num_steps):
            ...
            # 3) Phase metrics from active semantic contact (not stale wrapper face)
            ...
            active_cs = semantic.active_config.contact_strategy
            eef_to_contact = float(np.linalg.norm(
                eef_pos - _desired_contact_from_strategy(real_wrapper, active_cs)
            ))
            phase_metrics = {
            ...
            # 4) Check phase transition (may update active_config.contact_strategy)
