...
Phase 1 of FORTE models translational contact stiffness only. The estimator
tracks a 3x3 symmetric positive-definite matrix Sigma in task coordinates and
updates it online using the affine-invariant Riemannian geometry described in
the proposal.
"""
...
class RiemannianStiffnessEstimator:
    ...
    def __init__(
        ...
        if np.any(vals <= 0):
            raise ValueError(
                f"K_init is not positive definite (min eigenvalue={vals.min():.6e})"
            )
        self.eta = float(eta)
    ...
    def update(self, F_meas: np.ndarray, delta_x: np.ndarray) -> np.ndarray:
        """Perform one SPD(3) exponential-map update."""
    ...
    def predict_force(self, delta_x: np.ndarray) -> np.ndarray:
        """Predict task-frame force for a given task-frame displacement."""
        delta_x = np.asarray(delta_x, dtype=np.float64).ravel()
        if delta_x.shape != (3,):
            raise ValueError(f"delta_x must be (3,), got {delta_x.shape}")
    ...
    def _clip_symmetric_spectrum(self, M: np.ndarray, max_abs_eval: float) -> np.ndarray:
        vals, vecs = np.linalg.eigh(0.5 * (M + M.T))
        vals = np.clip(vals, -max_abs_eval, max_abs_eval)
        return vecs @ np.diag(vals) @ vecs.T
