...
from libero.libero.envs.arenas import TableArena
from libero.libero.envs.utils import rectangle2xyrange


@register_problem
class My_Tabletop_Manipulation(BDDLBaseDomain):
    def __init__(self, bddl_file_name, *args, **kwargs):
        ...
        # Optional per-env y-shift for the robot base to avoid interference
        # Negative values move the base along -y.
        self._robot_base_y_shift = kwargs.pop("robot_base_y_shift", 0)
        # Hard-code the robot to our custom mounted Panda for this env
        kwargs["robots"] = ["MyMountedPanda"]
        ...
        # Optional: per-object property overrides supplied at env construction
        # Example: {"card_1": {"size": (0.08, 0.06, 0.003), "density": 5}}
    ...
    def _reset_internal(self):
        # Default internal reset behavior
        super()._reset_internal()

    def _load_model(self):
        """
        Custom load to apply a base y-shift only for this tabletop env.
        """
        # For non-table arenas (unlikely here), defer to base behavior
        if getattr(self, "_arena_type", "table") != "table":
            return super()._load_model()

        # Initialize base robot structures without BDDLBaseDomain's arena / task assembly
        # (call the parent of BDDLBaseDomain, i.e., SingleArmEnv._load_model)
        super(BDDLBaseDomain, self)._load_model()

        # Rebuild arena / task with our custom base offset on y
        xpos = self.robots[0].robot_model.base_xpos_offset["table"](
            self.table_full_size[0]
        )
        # Apply y shift
        xpos = [xpos[0], xpos[1] + float(self._robot_base_y_shift), xpos[2]]
