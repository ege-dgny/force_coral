...
from libero.libero.envs.arenas import EmptyArena
from robosuite.models.tasks import ManipulationTask


@register_problem
class My_Floor_Manipulation_Shifted_Robot(BDDLBaseDomain):
    ...
    def _load_model(self):
        """
        Custom load that shifts the robot base by -0.50 along y for this scene.
        """
        # Only customize for floor arena; otherwise defer to base behavior
        if getattr(self, "_arena_type", "floor") != "floor":
            return super()._load_model()

        # Initialize robot model (but not arena / task) via SingleArmEnv
        super(BDDLBaseDomain, self)._load_model()

        # Get default base position for empty arena and shift y by -0.50
        xpos = self.robots[0].robot_model.base_xpos_offset["empty"]
        xpos = [xpos[0] + 0.10, xpos[1] - 0.50, xpos[2]]
        self.robots[0].robot_model.set_base_xpos(xpos)

        # Build Empty arena and assemble task
